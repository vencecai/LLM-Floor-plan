 ...
 */
const RoomVisualizer = ({ floorPlanData, editor, boundaryData }) => {
  ...
  const drawRooms = () => {
    ...
    // 检查 TLDraw API
    if (
      typeof editor.createShape !== "function" &&
      typeof editor.create !== "function"
    ) {
    ...
    try {
      ...
      if (!jsonResult) {
        console.warn(
          "No JSON result data found, data structure:",
          floorPlanData
        );
        renderInProgressRef.current = false;
        return;
      ...
      // 创建新的房间形状
      const newShapeIds = layoutedRooms
        .map((room) => {
          ...
          if (!validColors.includes(color)) {
            console.warn(
              `Invalid color: ${color}, room: ${room.name}, using default value`
            );
            color = "white";
          }
          ...
          try {
            ...
            if (typeof editor.createShape === "function") {
              ...
              throw new Error(
                "No compatible create method found in TLDraw API"
              );
            }
